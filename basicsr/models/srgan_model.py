...
class SRGANModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('pixel_opt'):
            opt_ = train_opt['pixel_opt'].copy()
            pixel_type = opt_.pop('type')
            cri_pix_cls = getattr(loss_module, pixel_type)
            self.cri_pix = cri_pix_cls(**opt_).to(
                self.device)
        else:
            self.cri_pix = None

        if train_opt.get('perceptual_opt'):
            opt_ = train_opt['perceptual_opt'].copy()
            percep_type = opt_.pop('type')
        ...
        if train_opt.get('gan_opt'):
            opt_ = train_opt['gan_opt'].copy()
            gan_type = opt_.pop('type')
            cri_gan_cls = getattr(loss_module, gan_type)
            self.cri_gan = cri_gan_cls(**opt_).to(self.device)
    ...
    def setup_optimizers(self):
        train_opt = self.opt['train']
        # optimizer g
        opt_ = train_opt['optim_g'].copy()
        optim_type = opt_.pop('type')
        if optim_type == 'Adam':
        ...
        if (current_iter % self.net_d_iters == 0
            ...
            if self.cri_perceptual:
                ...
                if l_g_percep is not None:
                    l_g_total += l_g_percep * 0.01
                    loss_dict['l_g_percep'] = l_g_percep * 0.01
